...
//Referente a criptografia TEA
constexpr uint32_t ConexaoBD::delta;
constexpr uint32_t ConexaoBD::chave[4];

// Com base na criptografia TEA, o dado sera criptografado
uint64_t ConexaoBD::criptografarTEA(uint64_t dado){
    // Separo o dado em dois blocos de 32 bits
    uint32_t dadoA = static_cast<uint32_t>(dado >> 32); // parte alta, bits mais significativos
    uint32_t dadoB = static_cast<uint32_t>(dado & 0xFFFFFFFF); // parte baixa, bits menos significativos

    // util
    uint32_t soma = 0;

    // determino o numero de rodadas
    const int numRodadas = 32;

    // recalculo os novos valores dos dois blocos
    for(int i = 0; i < numRodadas; i++){
        soma += delta;
        dadoA += ((dadoB << 4) + chave[0]) ^ (soma + dadoB) ^ ((dadoB >> 5) + chave[1]);
        dadoB += ((dadoA << 4) + chave[2]) ^ (soma + dadoA) ^ ((dadoA >> 5) + chave[3]);
    }

    // uno de volta os dois blocos
    uint64_t dadoFinal = (static_cast<uint64_t>(dadoA) << 32) | dadoB;

    return dadoFinal;
}

// Com base na criptografia TEA, o dado sera descriptografado
uint64_t ConexaoBD::descriptografarTEA(uint64_t dado){
    // Separo o dado em dois blocos de 32 bits
    uint32_t dadoA = static_cast<uint32_t>(dado >> 32); // parte alta, bits mais significativos
    uint32_t dadoB = static_cast<uint32_t>(dado & 0xFFFFFFFF); // parte baixa, bits menos significativos

    // util
    uint32_t soma = delta << 5;
...
uint64_t ConexaoBD::toUint64(int num)
{
    uint64_t result = 0;
    std::memcpy(&result, &num, sizeof(int));
    return result;
}

uint64_t ConexaoBD::toUint64(double num)
{
    uint64_t result = 0;
...
uint64_t ConexaoBD::toUint64(QString str)
{
    return;
}

int ConexaoBD::toInt(uint64_t num)
{
    int result = 0;
    std::memcpy(&result, &num, sizeof(uint64_t));
    return result;
}

double ConexaoBD::toDouble(uint64_t num)
{
    double result = 0;
    std::memcpy(&result, &num, sizeof(uint64_t));
    return result;
...
QString ConexaoBD::toQString(uint64_t num)
{
    QString str = QString::fromUtf8(reinterpret_cast<const char*>(&num), sizeof(uint64_t));
    return;
}
...
double ConexaoBD::getSaldo(QString cpf)
{
    ...
    uint64_t valorRecuperado = 0;
    memcpy(&valorRecuperado, blob.constData(), sizeof(valorRecuperado));

    valorRecuperado = descriptografarTEA(valorRecuperado);

    return toDouble(valorRecuperado);
}
...
const bool ConexaoBD::setSaldo(QString cpf, double novoSaldo)
{
    ...
    query.addBindValue(criptografarTEA(toUint64(novoSaldo)));
    query.addBindValue(cpf);
